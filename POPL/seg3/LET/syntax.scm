...
(define the-lexical-spec
  '(
    ...
    (number     ((concat digit (arbno digit)))              number)
    (varname    (letter (arbno (or letter digit))) symbol)
   )
)
; Grammar
(define the-grammar
  '(
    ...
    (expression ("let" "(" varname "," expression ")") assign-exp)
    (expression (varname) eval-exp)
